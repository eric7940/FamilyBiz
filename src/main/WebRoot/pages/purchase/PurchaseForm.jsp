...
<title>
<logic:notEqual name="PURCHASE_BACK" scope="session" value="Y"><bean:message key="all.func.purchase.2"/></logic:notEqual>
<logic:equal name="PURCHASE_BACK" scope="session" value="Y"><bean:message key="all.func.purchase.4"/></logic:equal>
</title>
...
<bean:size id="sizeOfFacts" name="form" property="facts"/>
...
<input type="button" value="<bean:message key="purchase.btn.1"/>" onclick="resetDetails()">
<input type="button" value="<bean:message key="purchase.btn.2"/>" onclick="openFactList()">
</div>
...
<th width="100"><bean:message key="all.column.purchase.2"/></th><td width="100"><html:text property="purchaseDate" styleId="purchaseDate" size="9"/><script language="JavaScript">new tcal ({'controlname': 'purchaseDate'});</script>
</td>
</tr>
<tr>
...
<tr>
<th><bean:message key="all.column.fact.6"/></th><td id="addr" colspan="3"></td><th><bean:message key="all.column.purchase.4"/></th><td><html:text styleId="invoiceNbr" property="invoiceNbr" size="10"/></td>
</tr>
<tr>
...
<table class="sheet" cellspacing="0" cellpadding="1" border="1" width="650">
<tr>
<th width="100"><bean:message key="all.column.purchase.8"/></th><td width="150"><html:text styleId="discount" property="discount" onkeyup="convertNum(this);computeDiscount(this);" styleClass="num"/></td>
...
<tr>
<th><bean:message key="all.column.purchase.11"/></th><td colspan="3" height="50" valign="top"><html:text styleId="memo" property="memo" size="50"/></td>
</tr>
</table>
...
var factFlag = false;
var backFlag = '<bean:write name="PURCHASE_BACK" scope="session"/>';

$(document).ready(function(){
	openFactList(true);
	
	$("#purchaseDate").val(getToday());
	$("#amt").html("0");
	$("#total").html("0");
	
});

function openFactList(initFlag) {
	if (initFlag == null) initFlag = false;
	
	if (!initFlag && $("#factId").html() != '' && !confirm('重新選擇廠商時，進貨資料將會清空。你確定嗎？'))
		return;
		
	var src = '<html:rewrite page="/purchase.do"/>';
	var rtnValue = openDialogWindow(src, '',"getFactList", 700, 200);
...
function addDetailRow(){
	...
	var cellData = [
		...
		{id: null, html: '<input type="text" name="detail-' + objIdx + '-price" id="detail-' + objIdx + '-price" size="10" value="" onblur="computeDetailAmt(this, \'price\')" onkeyup="convertNum(this)" class="num"/>'},
	...
	$("#detail-" + objIdx + "-prodNme").autocomplete('<html:rewrite page="/purchase.do"/>', {
		matchContains: true,
		minChars: 0,
		width: 260,
		cacheLength: 1,
